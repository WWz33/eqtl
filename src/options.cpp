...
void print_help() {
  // single-binary, mainstream-bio-cli-style: behavior only
  std::cout
    ...
    << "\n"
    << "Input options:\n"
    << "    -v, --vcf FILE         genotypes VCF/BCF\n"
    << "    -e, --pheno FILE       phenotype matrix (sample x gene)\n"
    << "    -g, --gff FILE         gene annotation GFF3 (TSS)\n"
    << "    -c, --covar FILE       covariates (optional)\n"
    << "    -k, --grm PREFIX       GCTA GRM prefix (.grm.id/.grm.bin)\n"
    << "        --gff-id-key STR   GFF attribute key for gene id\n"
    << "\n"
    << "Mode options:\n"
    << "        --make-grm         write GRM from VCF and exit\n"
    << "    -m, --mode STR         cis|trans|all  [all]\n"
    << "        --model STR        lm|glm|lmm|glmm[,...]  [lmm]\n"
    << "    -w, --window INT       cis window bp around TSS  [1000000]\n"
    << "        --pval-cis FLOAT   cis write threshold  [1e-5]\n"
    << "        --pval-trans FLOAT trans/gw write threshold  [1e-5]\n"
    << "        --miss-hand STR    filter|impute  [filter]\n"
    << "        --fast             per-gene shared nuisance (lmm/glm/glmm)\n"
    << "\n"
    << "Permutation options:\n"
    << "        --perm INT         permutations for gene-level p  [10000]\n"
    << "        --seed INT         RNG seed for permutations\n"
    << "        --disable-beta-approx  write p_emp only\n"
    << "\n"
    << "Output options:\n"
    ...
    << "\n"
    << "Examples:\n"
    << "    eqtl -v data/smoke.vcf.gz -e data/smoke.pheno.tsv -g data/smoke.gff -o out --model lm --mode cis --perm 0\n"
